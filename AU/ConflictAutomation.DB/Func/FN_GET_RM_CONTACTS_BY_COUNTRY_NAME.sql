-- FN_GET_RM_CONTACTS_BY_COUNTRY_NAME.sql
-- 2024-06-18-18-08UTC
...
CREATE OR ALTER FUNCTION [dbo].[FN_GET_RM_CONTACTS_BY_COUNTRY_NAME]
(	
...
BEGIN
	...
	SELECT TOP 1 @RESULT = RMContact FROM CAU_SanctionsContact WHERE WFAR_Country = @COUNTRY_DESCRIPTION
	IF(ISNULL(@RESULT, '') <> '') BEGIN
		RETURN ISNULL(@RESULT, '')
	END

	-- 2nd Try: look for the country name as a substring inside CAU_SanctionsContact.WFAR_Country
	SELECT TOP 1 @RESULT = RMContact FROM CAU_SanctionsContact WHERE WFAR_Country LIKE '%' + @COUNTRY_DESCRIPTION + '%'
	IF(ISNULL(@RESULT, '') <> '') BEGIN
		RETURN ISNULL(@RESULT, '')
	END
	...
	-- 3rd Try: look for 'All countries' in CAU_SanctionsContact.Country for the corresponding Area/Region
		...
		-- 3.3. Get the RMContact corresponding to "All countries" in the corresponding @AREA and @REGION
		SELECT @RESULT = RMContact FROM CAU_SanctionsContact WHERE WFAR_Area = @AREA AND WFAR_Region = @REGION AND WFAR_Country = 'All countries'
